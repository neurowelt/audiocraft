...
class MusicGen:
    ...
    def get_pretrained(name: str = 'melody', device: tp.Optional[str] = None, 
        ...
        if name not in _HF_MODEL_CHECKPOINTS_MAP:
            raise ValueError(
                f"{name} is not a valid checkpoint name. "
                f"Choose one of {', '.join(_HF_MODEL_CHECKPOINTS_MAP.keys())}"
            )
        ...
        compression_model = load_compression_model(model_location, device=device, cache_dir=None)
        lm = load_lm_model(model_location, cfg_path=cfg_path, device=device, cache_dir=None)
        if name == 'melody':
